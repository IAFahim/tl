...
namespace UnifiedTimelineProof
{
    ...
    public struct StageReceiptComponent : IComponentData { public int DamageCalls; public int AnimationCalls; }
    ...
    public partial struct ClearTimelineStagesJob : IJobEntity
    {
        ...
        {
            timeline.Value = Timeline.ClearSelection(in timeline.Value);
    ...
    [BurstCompile(CompileSynchronously = true)]
    [WithOptions(EntityQueryOptions.IgnoreComponentEnabledState)]
    [WithAll(typeof(ResistanceComponent), typeof(HealthComponent), typeof(PoseComponent), typeof(StageReceiptComponent))]
    public partial struct SelectTimelineJob : IJobEntity
    {
        ...
        private void Execute(ref TimelineComponent timeline, EnabledRefRW<DamageActive> damage, EnabledRefRW<AnimationActive> animation)
        {
            if (timeline.Value.Asset.DamageOnly)
                return;
    ...
    [BurstCompile(CompileSynchronously = true)]
    [WithOptions(EntityQueryOptions.IgnoreComponentEnabledState)]
    [WithAll(typeof(ResistanceComponent), typeof(HealthComponent), typeof(StageReceiptComponent))]
    public partial struct SelectDamageOnlyTimelineJob : IJobEntity
    {
        public uint GameTick;
        public int Direction;

        private void Execute(ref TimelineComponent timeline, EnabledRefRW<DamageActive> damage)
        {
            if (!timeline.Value.Asset.DamageOnly)
                return;
            timeline.Value = Timeline.Select(in timeline.Value, GameTick, Direction);
    ...
    {
        private void Execute(in TimelineComponent timeline, in ResistanceComponent resistance, ref HealthComponent health, ref StageReceiptComponent receipt)
        {
            receipt.DamageCalls++;
    ...
    {
        private void Execute(in TimelineComponent timeline, in HealthComponent health, ref PoseComponent pose, ref StageReceiptComponent receipt)
        {
            receipt.AnimationCalls++;
    ...
    public partial struct CombatSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            for (var remaining = delta; remaining != 0; remaining -= direction)
            {
                ...
                dependency = new ClearTimelineStagesJob().ScheduleParallel(dependency);
                dependency = new SelectDamageOnlyTimelineJob { GameTick = gameTick, Direction = direction }.ScheduleParallel(dependency);
    ...
    public static class CombatEntities
    {
        public static Entity Create(EntityManager manager, TimelineAsset asset, in Resistance resistance, in Health health, in Pose pose)
        {
            ...
            manager.SetComponentData(entity, new StageReceiptComponent());
