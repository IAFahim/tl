...
var generated = Emit(plan, regions, operands);
...
static string Emit(TimelinePlan plan, EmittedWorkSlot[][] regions, Operands operands)
{
    ...
    Line(writer);
    EmitBatch(writer, plan, regions, operands, false);
    Line(writer);
    EmitScalar(writer, plan, regions, operands, true);
    Line(writer);
    EmitBatch(writer, plan, regions, operands, true);
    Line(writer);
    Line(writer, "    [MethodImpl(MethodImplOptions.AggressiveInlining)]");
...
static void EmitBatch(StringBuilder writer, TimelinePlan plan, EmittedWorkSlot[][] regions, Operands operands, bool backward)
{
    var name = backward ? "Backward" : "Forward";
...
static void RunSelfTests(string path, string source)
{
    ...
    var operands = ValidateGrammar(definition, source);
    var plan = RegionAnalyzer.Analyze(definition);
    _ = Emit(plan, WorkSlotMaterializer.ForRegions(plan), operands);
...
sealed record Options(string? Source, string? Output, bool SelfTest)
{
    public static Options Parse(string[] args)
    {
        ...
        return new Options(source, output, selfTest);
    }
}
