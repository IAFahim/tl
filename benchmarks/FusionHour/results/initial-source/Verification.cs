    ...
    public static void Run()
    {
        ...
        try
        {
            ...
            foreach (var backward in new[] { false, true })
            {
                ...
                var sequence = Enumerable.Range(0, 4096).Select(static i => (uint)i).ToArray();
                var random = new uint[4096];
                ...
                foreach (var ticks in new[] { sequence, sequence.Reverse().ToArray(), random, Enumerable.Repeat(321u, 4096).ToArray() })
                {
