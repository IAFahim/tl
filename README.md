...
- **Safe reclamation** — immovability is the reclamation proof: per-index blocks, retired directories, motion words, and dedupe-table arrays are freed only in `Dispose`, so there is no late reclamation, no graveyard, and no epoch a consumer must track.
