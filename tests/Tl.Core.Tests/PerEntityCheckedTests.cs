...
public class PerEntityCheckedTests
{
    ...
    public void SetAdvanceRejectsMismatchedColumns()
    {
        ...
        set.Add(looping);
        Assert.Throws<ArgumentException>(() => set.Advance([ 0 ], [ 0, 1 ], [ 0 ], true));
        Assert.Throws<ArgumentException>(() => set.Advance([ 0, 0 ], [ 0, 1 ], [ 0, 1, 2 ], true));
    }

    [Fact]
    public void TimelineAdvanceRejectsMismatchedColumns()
    {
        Assert.Throws<ArgumentException>(() => Timeline.Advance([ 0 ], [ 0, 1 ], [ 0, 1 ], true));
        Assert.Throws<ArgumentException>(() => Timeline.Advance([ 0, 0 ], [ 0, 1 ], [ 0 ], true));
    }
}
#endif
