...
internal static unsafe class InstalledPairs
{
    ...
    private static void AlphaExecute(byte* slot, byte* pair, ushort tick, FrameFlags flags, void** columns, int row)
    {
        AlphaClip scratch = default;
        var current = TickFrame.ToFrame<AlphaTrack, AlphaClip>(slot, pair, tick, flags, ref scratch);
        var health = (Health*)columns[0];
        if (health != null)
            health[row].Value += current.Clip.Value * current.Track.Code;
    }

    private static void BetaExecute(byte* slot, byte* pair, ushort tick, FrameFlags flags, void** columns, int row) => throw new InvalidOperationException("Beta consumer failed.");
