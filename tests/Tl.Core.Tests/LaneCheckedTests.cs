...
public partial class LaneTests
{
    ...
    public void SetThrowsAfterDispose()
    {
        ...
        var lane = timelines.Gather([ 0 ]);
        timelines.Dispose();

        Assert.Throws<ObjectDisposedException>(() => timelines.Gather([ 0 ]));
        var threw = false;
        try { lane.Seek([ 0 ], true).Apply(new float[1]); }
    ...
    public void SetDisposeToleratesEmptyAndRepeatedDispose()
    {
        ...
        populated.Dispose();
        populated.Dispose();
        Assert.Throws<ObjectDisposedException>(() => populated.Gather([ 0 ]));
    }
}
#endif
