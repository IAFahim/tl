...
internal static unsafe class TimelineKernel_697d1806343a75b35ab9edea6c84a83cfcbc87bad6e328c292343be147d2f075
{
    ...
    static unsafe bool Tick(byte* asset, int* heads, void** columns, TimelineComponent* rows, int rowCount, uint gameTick, int delta)
    {
        ...
        if (rowCount == 1)
        {
            ...
            while (remaining-- != 0)
            {
                ...
                if (!TimelineMovement.Select(state, 1u, false, isReverse, out var multiNext, out var multiTick, out var multiCycle, out var multiFlags)) break;
    ...
    static unsafe bool TickUniform(byte* asset, int* heads, void** columns, TimelineComponent* rows, int rowCount, uint gameTick, int delta, uint uniformPosition, long uniformCycle, long uniformPositionWord)
    {
        ...
        while (moved && multiRemaining-- != 0)
        {
            ...
            moved = false;
            if (TimelineMovement.Select(new TimelineState(1, uniformPosition, uniformCycle), 1u, false, multiReverse, out var uniformNext, out var uniformTick, out var uniformOutCycle, out var uniformFlags))
                {
                    moved = true;
    ...
    static unsafe bool TickMixed(byte* asset, int* heads, void** columns, TimelineComponent* rows, int rowCount, uint gameTick, int delta)
    {
        ...
        while (moved && multiRemaining-- != 0)
        {
            ...
            for (var row = 0; row < rowCount; row++)
            {
                ...
                memoSelected = TimelineMovement.Select(new TimelineState(1, memoPosition, memoCycle), 1u, false, multiReverse, out var memoNext, out memoTick, out memoOutCycle, out memoFlags);
            ...
            for (var row = 0; row < rowCount; row++)
            {
                ...
                memoPosition = component->Position;
                memoCycle = component->Cycle;
                memoSelected = TimelineMovement.Select(new TimelineState(1, memoPosition, memoCycle), 1u, false, multiReverse, out var commitNext, out _, out _, out _);
