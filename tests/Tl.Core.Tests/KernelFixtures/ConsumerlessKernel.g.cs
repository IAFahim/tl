...
internal static unsafe class TimelineKernel_f1845eb94725dfb7eb0437148041742461e15c5556bb9d6f330645d39724eb01
{
    [ModuleInitializer]
    internal static void Install() =>
        TimelineKernels.Register(0xB7DF2547B95E84F1, 0x24744180143704EB, 0x6F9DBB56555CE161, 0x01EB2497D3450633, &Tick);
    ...
    static unsafe bool Tick(byte* asset, int* heads, void** columns, TimelineComponent* rows, int rowCount, uint gameTick, int delta)
    {
        ...
        if (rowCount == 1)
        {
            ...
            {
                var reverse = delta < 0;
                if (TimelineMovement.Select(state, 2u, false, reverse, out var next, out var tick, out var cycle, out var flags))
                {
            ...
            while (remaining-- != 0)
            {
                ...
                if (!TimelineMovement.Select(state, 2u, false, isReverse, out var multiNext, out var multiTick, out var multiCycle, out var multiFlags)) break;
    ...
    static unsafe bool TickMixed(byte* asset, int* heads, void** columns, TimelineComponent* rows, int rowCount, uint gameTick, int delta)
    {
        ...
        while (moved && multiRemaining-- != 0)
        {
            ...
            for (var row = 0; row < rowCount; row++)
            {
                ...
                memoSelected = TimelineMovement.Select(new TimelineState(1, memoPosition, memoCycle), 2u, false, multiReverse, out var memoNext, out memoTick, out memoOutCycle, out memoFlags);
            ...
            for (var row = 0; row < rowCount; row++)
            {
                ...
                memoPosition = component->Position;
                memoCycle = component->Cycle;
                memoSelected = TimelineMovement.Select(new TimelineState(1, memoPosition, memoCycle), 2u, false, multiReverse, out var commitNext, out _, out _, out _);
