    ...
    static unsafe void Tick(byte* asset, int* heads, void** columns, TimelineComponent* rows, int rowCount, uint gameTick, int delta)
    {
        ...
        if (rowCount == 1)
        {
            ...
            while (remaining-- != 0)
            {
                ...
                if (!TimelineMovement.Select(state, 2u, false, isReverse, out var next, out var tick, out var cycle, out var flags)) break;
        ...
        while (moved && multiRemaining-- != 0)
        {
            ...
            for (var row = 0; row < rowCount; row++)
            {
                ...
                if (TimelineMovement.Select(new TimelineState(1, component->Position, component->Cycle), 2u, false, multiReverse, out _, out var tick, out var cycle, out var flags))
                {
                    moved = true;
            ...
            {
                var component = rows + row;
                if (TimelineMovement.Select(new TimelineState(1, component->Position, component->Cycle), 2u, false, multiReverse, out var next, out _, out _, out _))
                {
