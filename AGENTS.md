...
- Project and issue-comment transitions are serialized by `refs/heads/issue-transactions/<issue>`. Never recover a live transaction. After confirmed process or machine loss, use `eng/agent-work recover-lock <issue> <exact-transaction> <reason>` before resuming or taking over work.
