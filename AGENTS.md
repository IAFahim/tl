...
`tl` compiles immutable, heterogeneous timelines into deterministic execution. The v1.0.0-alpha.3 contract is owned by issue #27 and `plan.md`: pure timeline selection, ordered typed operation jobs over borrowed component storage, and total signed Tick. Default playback is ready; finite timelines clamp independently; every available crossed frame executes. .NET and Unity share domain operation signatures while retaining host-specific scheduling and storage adapters. This contract supersedes issue #16 for the breaking API migration; existing production still exposes the previous Start/TrySeek surface until that migration passes its gates. Never describe a handwritten reference or a design as shipped generator output. Stable asset identities and generated schema contracts are distinct from process-local routing IDs. New C backend work remains deferred. Authoring/import validates definitions before execution; reflection, hidden allocation, and implicit runtime compilation are outside the compiled path.
