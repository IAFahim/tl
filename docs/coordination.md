...
Recovery itself compare-and-set replaces the old transaction, records the audit on the issue, and releases the replacement. If audit publication fails, the replacement stays locked and the same command with the original object resumes it. A wrong, unrelated, or changed object is rejected. Never recover a live transaction merely because a command is slow.
