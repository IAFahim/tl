...
- **Range-batched consumer calli plus lockstep memoization in the kernel warm path.** One `calli` per contiguous same-state row run and one `Select` per distinct (position, cycle) cut the 100k-row kernel arm from 790 to ~195 us/tick (4.1x) with checksum-identical effects and 0 B retained; uniform row sets run one `Select` and one consumer run for the whole set. ([kernel-warm-path-perf.md](kernel-warm-path-perf.md))
- **Fused kernel validation returning `bool` beats a managed pre-scan.** Moving the row-address/uniformity probe inside the generated `Tick` with an interpreter fallback removed the per-tick managed scan (visible at 100k rows) and halved the realistic single-row Read+Write tick (751 -> 368 ns). The single-row body must stay inline in `Tick`; behind a separate call it cost a constant ~2 ns/tick and regressed the minimal Alpha lane +15-25%. ([kernel-warm-path-perf.md](kernel-warm-path-perf.md))
- **Bind-hash normalization, fused single-pass mixed commit, and hoisted consumer range pointers (issue #87).** Hashing the `Strip`-normalized hot view in `TimelineKernels.Find` made `tlbake --kernel` products bind at all (metadata assets previously always ran the interpreter), committing memo rows inline deleted the mixed path's second full sweep, and one `Range` resolution per `Tick` replaced per-run global-table loads in the generated `Run`. Same-machine mixed harness (1M staggered rows): facade-with-metadata 9.04-9.27 -> 5.60-5.70 ns/row/tick (1.6x), already-bound kernels 7.50-7.53 -> 5.51-5.59 (1.35x), interpreter control unchanged; the 2.31 ns/row decomposition lane quoted on the issue was a scratch harness outside the repository and did not transfer. ([kernel-warm-path-perf.md](kernel-warm-path-perf.md))
