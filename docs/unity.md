...
The stable lane uses Unity 6000.0.83f1, Entities 1.4.3, and Burst 1.8.30. The preview lane uses Unity 6000.7.0a5, Entities 6.7.0, Collections 6.7.0, and Burst 2.0.0. Both lanes pass 4/4 EditMode and 4/4 PlayMode tests. The exact package README example compiles through the package-only materializer.

The stable 10,000-row fixture schedules 20 jobs per step, matches its independent oracle, measures 18.006 ns/entity-step, and allocates 0 managed B on the main thread after warmup. Its Mono and IL2CPP Linux players execute the generalized clear, select, operation, and commit jobs, contain the expected Burst symbols, print `TL_UNITY_PLAYER_OK`, and contain no TL compiler, generator, Roslyn, editor hash shim, or test assembly. Native leak scans pass.

Build the deterministic UPM archive into an empty directory with:

```sh
eng/package-unity /tmp/tl-unity-package
```

The archive contains exactly 13 approved entries. The release pipeline builds it twice, rejects duplicate or additional archive paths, checks package identity and deterministic gzip metadata, and records it in the outer manifest and checksums. Full receipts and exact tool commands are retained in [issue #34](https://github.com/IAFahim/tl/issues/34).
