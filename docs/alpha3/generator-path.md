...
Semantic slot identity is `(declared role, neutral type)`, not CLR type or access mode alone. Access is a per-operation effect, and the catalog computes the joined access for storage and scheduling. `Damage(ref Health health)` followed by `Animation(in Health health)` borrows one `health` column and retains the stage's read-after-write dependency. By default the parameter name is the declared role, so `bodyPose` and `aimPose` remain distinct columns even when both have type `Pose`. An ambiguous rename, same role with conflicting type, or incompatible effect join is a generation diagnostic before execution; only an explicit reviewed binding may alias different roles.
...
For Unity, distinct roles of the same value type become distinct generated role component wrappers, and the wrapped field is the canonical ECS storage. The operation receives `in` or `ref` directly to that field according to its own effect; no per-tick component copy is introduced. The generated setup API adds the correct role wrapper, and the chunk schema gate checks those wrapper types. Same-role/type operations share one wrapper using the catalog's joined access.
...
`experiments/Alpha3/Generator/ProposedShape.cs` compiles the proposed .NET runtime declarations, builders, jobs, timelines, and catalog. `GeneratedSurfacePlaceholder.cs` is explicitly handwritten and compiles the `Combat.Query` shape and complete consumer that TL would generate; it is not evidence that the current generator emits those members. The user's `Combat.Define` does not call any generated member. For .NET, the future TL generator can add its partial before the compiler consumes the application; no second generator needs to inspect that output.

This project targets the repository's .NET 10/C# toolchain. Its static-abstract interfaces, ref fields, records, and `scoped` scaffolding do not prove that identical declaration source compiles in the qualified Unity host. The Unity materializer must emit its supported C# subset, and the shared domain `Execute` declaration and body must themselves stay within that subset. The settings-first inference law is ordinary C# and was independently compiled by the manager's C# 9 builder proof.
...
One entity timeline owns one shared clock even when it contains many track/clip/job pairings. For each simulated frame, the generated coordinator selects that row once, schedules every typed occurrence through the authored dependency chain, and commits once after all required work completes. Delta `N` repeats this whole sequence `N` times. Per-pair .NET kernels and Unity `IJobEntity` wrappers never own or advance playback. A track with no active clip contributes no occurrence to that shared selection; it does not acquire an independent position. Every occurrence receives common game tick, timeline tick, Cycle, direction, and timeline-boundary metadata from the same pending selection, while clip-local flags and payload come from that occurrence.
...
This gate is recomputed after the host's structural-change dependencies on every tick; it does not trust enableable markers left from a previous archetype. A structural change moves an entity to a new chunk, and the next selection pass derives membership from that chunk. The lower-bound gate cost is `O(chunks * C + E)`, where `C` is the distinct component-type set in the catalog and `E` is its timeline rows, before stage dispatch. Caching a chunk schema fingerprint is an optimization only if invalidation by Unity's structural version is proven.

Ordinary `RegisterSourceOutput` cannot make a TL-emitted wrapper visible to Entities generation in the same compilation, so deterministic precompile materialization remains the selected Unity path. Roslyn also documents experimental [`RegisterPreCompilationSourceOutput`](https://github.com/dotnet/roslyn/blob/main/docs/features/incremental-generators.cookbook.md#pre-compilation-source-generation), but it is restricted to non-compilation inputs and carries `RSEXPERIMENTAL007`. It cannot use `SyntaxProvider` or `CompilationProvider` to inspect the authored `Execute` signatures. TL's `netstandard2.0` analyzer targets Roslyn 4.3.1, so that facility is an unqualified future probe rather than a dependency for this migration.
...
- semantic slots by declared role and neutral type ID, plus each operation's access effect and the catalog's joined access;
...
7. **Receipts and migration.** Replace alpha.2 assertions in `SeekReaderTests.cs`, `GeneratedSeekTests.cs`, `GeneratedSeekRuntimeTests.cs`, `CompatibleRoutingTests.cs`, and the public API approvals. Extend `HeterogeneousCompositionTests.cs` for ordered repeats/blends and `IncrementalGeneratorTests.cs` for catalog-local invalidation and analyzer/CLI byte parity. Add damage-only versus invalid mixed-schema tests, default/empty/zero/clamp/loop extremes, A-B-A/opposing order/reverse/256 tracks, exception-before-commit, warmed allocation, and .NET/Unity oracle parity. Delay the second operation in an adversarial two-operation frame and verify that the first operation cannot expose advanced committed playback; completion becomes visible only after the delayed dependency finishes. Migrate `tests/Tl.Alpha` and `samples/Mixed` only after the new generated path is green.
