...
| Importer and baker | Validate assets and the authored duration, derive ordered schedules, canonicalize and deduplicate data |
...
Assets authored for the pre-v1 converter shape are rejected with versioned migration diagnostics, never silently reinterpreted: `removed property 'trackType'` (split into `namespace` + `type`), `removed property 'clipType'` (likewise), `removed property 'payload'` (renamed `data`), `removed property 'track'` (the pre-v1 grouping wrapper), and `renamed property 'loops'` (now `loop`); each message names the replacement field, and the removed-property messages additionally name `schema v1`. Track and clip elements each carry their full type identity and data — grouping is derived at bake, and execution order is authored clip order. Ship assets stripped (`tlbake --strip`), and audit distribution sizes with `tlbake --report`.
...
Consumers registered through `PairRuntime<TTrack, TClip>.Consume(execute, bind, TickPurity.WindowConstant)` additionally declare their per-tick output depends only on the slot's track and clip data — never on `Frame.TimelineTick`, `Frame.WithinClip`, or flags. `MeasuredLanes.Measure` then fills each clip window's table range from one evaluation per declared consumer, bit-identically to per-tick evaluation by construction: the fill batches only windows whose blended clip value is constant across the range (blend factor span at most 1), replays the captured per-consumer scalars at their exact program position (preserving accumulation order for mixed declared/undeclared stages), and keeps exact per-tick evaluation everywhere else, including crossfade windows. A declaration that lies — the consumer reads `TimelineTick`, `WithinClip`, or flags — is not detected: the table deterministically carries the window's first-tick evaluation, so tables diverge from per-tick playback. Undeclared registrations keep today's exact per-tick fill.
