...
The first portability backend is C11. Its contract becomes the stable native foundation for C++, Rust, engines, and FFI bindings. A neutral plan carries fixed-width IDs, windows, regions, movement facts, typed constant bytes, slot access, and stable operation IDs. C consumers implement the named operations; arbitrary C# method bodies are never presented as portable. The C ABI fixes layout, alignment, endianness, ownership, status values, and versioning before other language wrappers are added.
